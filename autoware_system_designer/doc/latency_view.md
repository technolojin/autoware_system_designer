...
- **Chain end**: how a group's chain stops is drawn at its last gate and named in the title. A _loop end_ is a return wire: out of the end of the gate, under the track and back along it, up into the start of the gate its message rejoins, labelled `↺` with that gate's name; the same wire leaves any gate on the chain whose message closes a loop further up. An _open end_ is a short stub with an end stop after the last gate, labelled under the block with the message nobody takes: an output no node subscribes to, an input that triggers no process, or a gate that publishes nothing. A _queued end_ is a stub into a slotted box: the last gate filled a queue (`to_queue`), and the chain hands off there to the processes reading it, named after `⇥`; those readers run on their own triggers, so no time is composed across the queue. A chain the hop limit cuts trails off without a stop. Clicking any of them opens a panel naming the two events of the loop, the readers of the queue, or the reason the chain is open.
- **Rate**: the design's declared rate, propagated from the clocks, labels every gate in the logical state. In the measured state the group title, the gate tooltip and the panels put the rate the run observed beside it: a port's from its take or publish count, a periodic gate's from the timer nearest its declared period, any other gate's from the first output it feeds. The design propagates the fastest trigger's rate through `and` and `or` gates alike, so a distortion corrector that declares a 50 Hz IMU among its triggers is declared at 50 Hz while the run shows the lidar rate; the panel flags a difference of more than a fifth. A queue stops the propagation: a corrector that queues the IMU and reads the queue on the cloud callback is declared at the lidar rate.

## States

The vertices are the process, queue and port events the node designs declare; clock roots are the `periodic` gates. A queue read is kept beside the trigger edges as its own relation and never walked. Every chain is analyzed from the design graph in every state; what changes is the time on it.
...
- **queue**: a chain end and no gate; its filler's chain stops at it with the filler's time, and a reader's chain starts from the reader's own trigger with nothing carried across.
