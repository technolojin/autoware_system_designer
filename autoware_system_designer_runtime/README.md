...
A system on `/clock` (`use_sim_time`, every simulation mode) declares its rates and runs its timers in ROS time, so its durations and rates are counted there: the tracer records every ROS time override a process receives, the analysis maps wall time onto that curve, and `run.clock` names the base (`ros` with the rate against wall time and the sample count, or `wall`). Records stay ordered and matched in wall time; only durations change. A paused bag makes ROS time stand still, so nothing inside the pause counts. `--measure-clock wall` measures a simulation in wall time; `auto` falls back to wall time when the trace holds no `/clock` samples.
